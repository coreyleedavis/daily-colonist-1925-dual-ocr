...
Dual-pipeline OCR testbed for the 1925 *Daily Colonist* (6,647 newspaper pages): Tesseract 5 and the PaddleOCR-VL vision-language model on identical microfilm scans, indexed in Solr with word-level highlighting, compared side by side in a IIIF/Mirador viewer, plus AI-generated image descriptions that make photos and ads searchable.

**Institution:** University of Victoria Libraries
**Author:** [FILL]
**Companion article:** [FILL — Code4Lib draft link]
**License:** [FILL — decision pending]

> **AI-assistance disclosure:** substantial portions of this codebase were written in AI-pair-development sessions with Claude Fable (Anthropic), under a verify-before-acting protocol described in the companion article. All code was executed and verified by the human author.
...
- **PaddleOCR-VL** (a ~0.9B-parameter vision-language model, served via vLLM): markedly better transcription, but block-level geometry only and no confidence reporting. A second model (Qwen2.5-VL-7B) generated natural-language descriptions of every image region — 39,458 descriptions — making photographs and advertisements searchable.
...
**Key numbers** (details and caveats in the companion article): Tesseract indexed 19.6M words with 748K unique forms; the VLM arm 26.8M words with 257K unique forms — at corpus scale, OCR noise appears as vocabulary inflation. The query *railway* matches 731 pages in the Tesseract arm and 2,7xx in the VLM arm (a count later found to be partially inflated by image-description text — see `analysis/check_*.py`, the investigation that led to Phase 2's stricter separation).
...
- **Not Phase 2.** The hybrid pipeline that fuses the two arms word-by-word with per-word provenance is a separate, later body of work described in the companion article.
...
Three approaches were tried and did not ship. They are included because negative results are results.

- **`experiments/chandra_ocr_year.py`, `chandra_one_page.py`** — an earlier attempt to use Chandra as the VLM arm. Whole-page decoding proved problematic (the failure modes are exactly those described in the companion article's architecture section), and an attempted repair using Surya for layout segmentation was unsuccessful on dense broadsheet (it segmented pages as a grid rather than by columns). The project moved to PaddleOCR-VL's integrated layout-then-recognize pipeline.
- **`experiments/rapl_sampler.py`** — a CPU-energy measurement tool (Intel RAPL counters) designed to run beside the Tesseract job and produce a measured energy figure for the classical arm. The protocol is sound and the tool works; no output from the production run survives, so the companion article's Tesseract energy figure is a labeled estimate. Rerunning a sampled hour under this tool would produce a measured figure.
- **`experiments/correct_text.py`, `correct_apply.py`** — an early post-OCR correction experiment (SymSpell against the 1925 lexicon) applied to VLM output. Abandoned; the project's direction moved from correcting transcripts toward re-reading and fusing them (see the companion article's related-work discussion of post-correction approaches).
...
2. **The VLM arm's indexed text includes AI-generated image descriptions.** This inflates its apparent match counts for some queries (discovered by `analysis/check_*.py`; quantified in the companion article). Description text is labeled in the demo but shares the text index in this phase.
...
If you use this code or approach, please cite the companion article: [FILL when published].

## Acknowledgements

[FILL — colleagues, UVic Libraries systems]. Scans digitized by the University of Victoria Libraries; hosted by the Internet Archive.
